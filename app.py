...
def evaluate_stock_data(ticker, df, min_vol):
    try:
        ...
        p, s50, s150, s200 = float(current_price), float(sma_50.iloc[-1]), float(sma_150.iloc[-1]), float(sma_200.iloc[-1])
        s200_20d = float(sma_200.iloc[-20]) if len(sma_200) >= 20 else s200
        
        low_52w, high_52w = float(df[price_col].tail(252).min()), float(df[price_col].tail(252).max())
        
        conditions = [
            bool(p > s150 and p > s200), bool(s150 > s200), bool(s200 > s200_20d),
            bool(s50 > s150 and s50 > s200), bool(p > s50), bool(p > low_52w * 1.25),
        ...
        # 專業 VCP 波動收縮檢測
        high_15, low_15 = float(df[price_col].tail(15).max()), float(df[price_col].tail(15).min())
...
async def process_screener(task_id: str, min_vol: int, min_cap: int, target_list: str):
    ...
    # 🔵 1. 平滑獲取大盤 1Y 基準報酬 (防止 0 報酬)
    benchmark_return = 15.0 # 預設保險絲
    try:
        sq = yq.Ticker("SPY")
        h = sq.history(period="1y")
        # 🔴 校驗：如果資料太少 (例如只有 1 筆今日資料)，則切換備援
        ...
        if not h.empty and len(h) > 10:
            ...
            p_c = 'adjclose' if 'adjclose' in h.columns else 'close'
            benchmark_return = float(((h[p_c].iloc[-1] / h[p_c].iloc[0]) - 1) * 100)
    except:
        benchmark_return = 15.0
    tasks_store[task_id]["benchmark"] = round(float(benchmark_return), 1)
    ...
    for i in range(0, len(market_tickers), batch_size):
        ...
        try:
            ...
            if all_history.empty:
                tasks_store[task_id]["progress"] += int(len(batch)); continue
    ...
    if not candidates:
        tasks_store[task_id]["status"] = "completed"; return
    ...
    for _, row in df_rank.iterrows():
        ...
        # 🟡 米奈維尼 Condition 8: 需贏過大盤 且 RS Rating >= 70
        cond_8 = (row['return_1y'] > benchmark_return) and (rs_rating >= 70)
        ...
        if target_list == "mag7":
            if row['ticker'] in mag7_list: final_results.append(display_obj)
        elif score == 8: 
            final_results.append(display_obj)

    tasks_store[task_id]["results"] = sorted(final_results, key=lambda x: (x['score'], x['rs_rating'], x['return_1y']), reverse=True)
    tasks_store[task_id]["status"] = "completed"
    tasks_store[task_id]["message"] = f"✅ 完成！已基於 {len(final_results)} 檔校正目標結果。"

@app.post("/api/screen/start")
async def start_screen(req: ScreenRequest, background_tasks: BackgroundTasks):
    task_id = str(uuid.uuid4())
    tasks_store[task_id] = { "status": "pending", "progress": 0, "total": 0, "results": [], "benchmark": 0.0, "message": "啟動中..." }
...
async def get_status(task_id: str):
    if task_id not in tasks_store: raise HTTPException(status_code=404, detail="Task not found")
